		...
		if (param && param instanceof Object) {
			...
			var defaults = new Object({
				...
				info : {
					...
					msg_on : true,
					link : '?page=',
					msg : '<span>&nbsp;&nbsp;跳转至第{currText}/{sumPage}页</span>',
					text : {
						width : '22px'
					}
				}
			...
			function getFirst() {
				...
				if (options.info && options.info.first_on && options.info.first) {
					var str = "<a href='" + getLink() + "1' title='1'>"
			...
			function getLast(pageCount) {
				...
				if (options.info && options.info.last_on && options.info.last) {
					var str = "<a href='" + getLink() + "" + pageCount
							+ "' title='" + pageCount + "'>"
							+ options.info.last + "</a>";
					return str
				} else {
			...
			function updateView() {
				...
				if (currPage == 1) {
					...
					content += "<a href='" + link + "" + (currPage - 1)
							+ "' title='" + (currPage - 1) + "'>" + getPrev()
							+ " </a>"
				}
				if (firstPage <= 0) {
					firstPage = 1
				...
				for (firstPage; firstPage < lastPage; firstPage++) {
					if (firstPage == currPage) {
						...
						content += "<a href='" + link + "" + firstPage
								+ "' title='" + firstPage + "'>" + firstPage
								+ "</a>"
					}
				}
